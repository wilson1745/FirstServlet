...
public class IPSeeker {
	...
	/**
	 * 從offset位置讀取4個字節為一個long，因為java為big-endian格式，所以沒辦法 用了這麼一個函數來做轉換
	 * 
	 * @param offset
	...
	/**
	 * 從offset位置讀取3個字節為一個long，因為java為big-endian格式，所以沒辦法 用了這麼一個函數來做轉換
	 * 
	 * @param offset
	 * @return 讀取的long值，傳回-1表示讀取檔案失敗
	 */
	...
	/**
	 * 從offset位置讀取四個字節的ip地址放入ip陣列中，讀取後的ip為big-endian格式，但是 檔案中是little-endian形式，將會進行轉換
	 * 
	 * @param offset
	 * @param ip
	 */
	...
	/**
	 * 這個方法將根據ip的內容，定位到包含這個ip國家地區的記錄處，傳回一個絕對偏移 方法使用二分法尋找。
	 * 
	 * @param ip 要查詢的IP
